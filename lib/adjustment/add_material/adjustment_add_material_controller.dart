...
class AdjustmentAddMaterialController extends GetxController {
  ...
  void getProductData() async {
    ...
    if (user != null) {
      String userId = user['id'].toString();
      var data = {"userid": userId};
      var res = await Network().post(data, '/core/adjustment-material');
      var body = jsonDecode(res.body);
      if (body['success']) {
  ...
  List<String> get dropdownProduct {
    ...
    for (var i = 0; i < productList.length; i++) {
      menuItems.add(productList[i]['material_name'] +
          ' - ' +
  ...
  void adjustmentStore(String transactionDate, List<String> productsId,
    ...
    if (user != null) {
      ...
      var data = {
        ...
        "md_material_id": productsId,
        "quantity": quantities,
        "type": types
      };
      var res = await Network().post(data, '/core/adjustment-material-store');
      var body = jsonDecode(res.body);
      if (body['success']) {
        loading(false);
