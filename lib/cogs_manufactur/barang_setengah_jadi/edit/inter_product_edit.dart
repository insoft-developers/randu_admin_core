...
import 'package:buzz/cogs_manufactur/barang_setengah_jadi/edit/inter_product_edit_controller.dart';
...
class InterProductEdit extends StatefulWidget {
  Map<String, dynamic> dataList;
  InterProductEdit({super.key, required this.dataList});

  @override
  State<InterProductEdit> createState() => _InterProductEditState();
}

class _InterProductEditState extends State<InterProductEdit> {
  final InterProductEditController _controller =
      Get.put(InterProductEditController());
  ...
  void initState() {
    _controller.getCategoryData().then((value) {
      ...
      _controller.selectedCategory.value =
          widget.dataList['category_name'].toString();
    });
    _controller.getUnitData().then((value) {
      _controller.selectedSatuan.value = widget.dataList['unit'];
    });

    for (var i = 0; i < widget.dataList['komposisi'].length; i++) {
      _controller.getMaterialData().then((value) {
        setState(() {
          _materials.add(
              widget.dataList['komposisi'][i]['material_name'].toString() +
                  ' - ' +
                  widget.dataList['komposisi'][i]['unit'].toString());
          _selectedMaterials.add(
              widget.dataList['komposisi'][i]['material_id'].toString() +
                  '_' +
                  widget.dataList['komposisi'][i]['product_type'].toString());
          _qty.add(TextEditingController(
              text: widget.dataList['komposisi'][i]['quantity'].toString()));
        });
      });
    }
    _itemDelete(0);
    // print(widget.dataList['komposisi'].length);
    // });

    _materialName.text = widget.dataList['product_name'].toString();
