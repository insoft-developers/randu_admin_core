...
class ProductManufactureAddController extends GetxController {
  ...
  void getProductData() async {
    ...
    if (user != null) {
      ...
      var res = await Network().post(data, '/core/product-manufacture-product');
  ...
  void getProductManufactureAccount() async {
    ...
    if (user != null) {
      String userId = user['id'].toString();
      var data = {"userid": userId};
      var res = await Network().post(data, '/core/product-manufacture-account');
      var body = jsonDecode(res.body);
      if (body['success']) {
  ...
  Future getMaterialData(String productId, String orderQty) async {
    ...
    var res = await Network().post(data, '/core/product-manufacture-change');
    var body = jsonDecode(res.body);
    if (body['success']) {
  ...
  void productManufactureStore(
    ...
    if (user != null) {
      ...
      var res = await Network().post(data, '/core/product-manufacture-store');
      var body = jsonDecode(res.body);
      if (body['success']) {
        loading(false);
        Get.back();
